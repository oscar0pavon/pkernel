...
// ============================================================================
static int nvme_cmd(NvmeQueue *q, uint8_t opc, uint32_t nsid, uint64_t prp1,
  ...
  *q->sq_db = q->sq_tail;
  ...
  for (uint32_t i = 0; i < 5000000; i++) {
    ...
    if (q->cq_head == 0)
      q->phase ^= 1; // phase flips on head wrap
    *q->cq_db = q->cq_head;
...
void nvme_probe(uint64_t mmio_base, volatile uint32_t *pci_regs) {
  if (nvme_drive_count >= NVME_MAX_DRIVES)
    return;
  ...
  for (uint32_t i = 0; i < 2000000; i++) {
    ...
    if (csts & 2) {
      printf("nvme: fatal status (CSTS=0x%x)\n", csts);
      return;
    }
    if (csts & 1)
      break;
  ...
  if (nvme_cmd(aq, 0x06, 0, (uint64_t)idbuf, 0, 0x01, 0, 0) != 0)
    return;
  ...
  if (nvme_cmd(aq, 0x05, 0, (uint64_t)mem->iocq, 0, 1U | ((qdepth - 1) << 16),
               1U, 0) != 0)
    return;
  ...
  if (nvme_cmd(aq, 0x01, 0, (uint64_t)mem->iosq, 0, 1U | ((qdepth - 1) << 16),
               1U | (1U << 16), 0) != 0)
    return;

  // ---- Identify Namespace 1 (CNS=0x00) ----
  memset(idbuf, 0, sizeof(idbuf));
  if (nvme_cmd(aq, 0x06, 1, (uint64_t)idbuf, 0, 0x00, 0, 0) != 0)
    return;
  ...
  // LBAF table at byte 128, each entry 4 bytes; byte 2 of entry = LBADS (2^n
  // bytes)
  ...
  // Expose this namespace through the generic block layer. NVME_MAX_DRIVES <=
  // 9, so a single digit suffix is sufficient.
  char name[6] = {'n', 'v', 'm', 'e', (char)('0' + idx), '\0'};
  block_register(name, blksz, nsze, (uint32_t)idx, NULL, nvme_block_read,
                 nvme_block_write);
}
...
int nvme_read(int drive, uint64_t lba, uint32_t count, void *buf) {
  ...
  int r = nvme_cmd(&ioq_q[drive], 0x02, 1, (uint64_t)nvme_rw_buf, 0,
                   (uint32_t)lba, (uint32_t)(lba >> 32), count - 1);
  if (r == 0)
    memcpy(buf, (void *)nvme_rw_buf, nb);
  return r;
}
...
int nvme_write(int drive, uint64_t lba, uint32_t count, const void *buf) {
  ...
  return nvme_cmd(&ioq_q[drive], 0x01, 1, (uint64_t)nvme_rw_buf, 0,
                  (uint32_t)lba, (uint32_t)(lba >> 32), count - 1);
}
