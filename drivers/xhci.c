...
// DMA receive buffer for USB descriptors (256 bytes covers any standard
// descriptor)
...
// Advance ep0_ring past slot ep0_enqueue-1, wrapping at the link TRB (slot
// 255).
...
// ============================================================================
// Configure slot count, scratchpad, DCBAAP, and start the controller
// (spec 4.2.2-3)
// ============================================================================
void xhci_start_controller(void) {
  ...
  uint32_t slots_en =
      (max_slots > max_dcbaa_slots) ? max_dcbaa_slots : max_slots;
  if (!slots_en)
    slots_en = 1;
...
void xhci_send_command(uint32_t trb_type, uint64_t parameter,
  ...
  trb->Status = 0;
  trb->Control =
      (trb_type << 10) | ((uint32_t)slot_id << 24) | command_ring_cycle;

  if (++command_ring_enqueue == 255) {
    command_ring[255].Control =
        (TRB_TYPE_LINK << 10) | (1 << 1) | command_ring_cycle;
...
uint32_t xhci_control_in(uint32_t slot_id, uint64_t setup,
  ...
  s->Status = 8;
  s->Control =
      (TRB_TYPE_SETUP_STAGE << 10) | (3U << 16) | (1U << 6) | ep0_cycle;
  ep0_advance();
...
// ============================================================================
// EP0 control transfer with no data stage (e.g. SET_PROTOCOL,
// SET_CONFIGURATION). Status Stage is IN (DIR=1) per xHCI spec 4.11.2.2.
// ============================================================================
uint32_t ep0_control_nodata(uint32_t slot_id, uint64_t setup) {
  ...
  s->Status = 8;
  s->Control =
      (TRB_TYPE_SETUP_STAGE << 10) | (0U << 16) | (1U << 6) | ep0_cycle;
  ep0_advance();
  ...
  t->Status = 0;
  t->Control =
      (TRB_TYPE_STATUS_STAGE << 10) | (1U << 16) | (1U << 5) | ep0_cycle;
  ep0_advance();
...
int xhci_address_device(uint32_t slot_id, uint32_t port) {
  ...
  input_ctx.ep[0].dw1 =
      (3U << 1) | (XHCI_EP_TYPE_CONTROL << 3) | ((uint32_t)mps << 16);
  ...
  if (!xhci_get_descriptor(slot_id, USB_DESC_DEVICE, 8))
    return 0;
  ...
  if (speed != 4 && reported_mps != mps)
    if (!xhci_evaluate_context(slot_id, reported_mps))
      return 0;

  if (!xhci_get_descriptor(slot_id, USB_DESC_DEVICE, 18))
    return 0;
  if (!xhci_get_config_descriptor(slot_id))
    return 0;

  uint8_t config_val = descriptor_buffer[5]; // bConfigurationValue
  if (!xhci_set_configuration(slot_id, config_val))
    return 0;
...
int xhci_get_descriptor(uint32_t slot_id, uint8_t desc_type, uint16_t length) {
  uint64_t setup = (uint64_t)0x80 | ((uint64_t)0x06 << 8) |
                   ((uint64_t)desc_type << 24) | ((uint64_t)length << 48);
...
int xhci_evaluate_context(uint32_t slot_id, uint8_t new_mps) {
  ...
  input_ctx.ep[0].dw1 =
      (3U << 1) | (XHCI_EP_TYPE_CONTROL << 3) | ((uint32_t)new_mps << 16);
...
// Parses Interface, HID, and Endpoint descriptors to populate iface_* /
// ep1_in_*.
// ============================================================================
int xhci_get_config_descriptor(uint32_t slot_id) {
  // Phase 1: 9-byte header to read wTotalLength
  uint64_t setup9 = (uint64_t)0x80 | ((uint64_t)0x06 << 8) |
                    ((uint64_t)USB_DESC_CONFIG << 24) | ((uint64_t)9 << 48);
  ...
  uint16_t total_length =
      (uint16_t)descriptor_buffer[2] | ((uint16_t)descriptor_buffer[3] << 8);
  ...
  uint64_t setup_full = (uint64_t)0x80 | ((uint64_t)0x06 << 8) |
  ...
  while (offset + 1 < total_length) {
    ...
    if (bLength < 2 || offset + bLength > total_length)
      break;
...
int xhci_set_configuration(uint32_t slot_id, uint8_t config_val) {
  ...
  printf("USB: slot %d class 0x%x proto 0x%x ep%d mps %d intv %d->%d\n",
         slot_id, iface_class, iface_protocol, ep1_in_number, ep1_in_mps,
         ep1_in_interval, xhci_interval);
  ...
  epc->dw1 = (3U << 1) | (XHCI_EP_TYPE_INTERRUPT_IN << 3) |
             ((uint32_t)ep1_in_mps << 16);
...
static int xhci_setup_msix(volatile uint32_t *pci, uint8_t cap_ptr,
  ...
  pci[cap_ptr / 4] =
      (cap_dw | (1U << 31)) & ~(1U << 30); // Enable, clear Function Mask
  return 1;
}
...
// xHCI interrupter and USBCMD.INTE. Prefers MSI-X (0x11), falls back to MSI
// (0x05).
void xhci_enable_msi(uint8_t vector) {
  volatile uint32_t *pci = xhci_dev.pci_regs;
  if (!pci) {
    printf("ERROR: xHCI PCI regs not set\n");
    return;
  }
  ...
  int ok = 0;
  if (msix_ptr)
    ok = xhci_setup_msix(pci, msix_ptr, vector);
  else if (msi_ptr)
    ok = xhci_setup_msi(pci, msi_ptr, vector);
  else {
    printf("ERROR: neither MSI-X nor MSI capability found\n");
    return;
  }
  if (!ok)
    return;
...
void init_xhci_driver(void) {
  ...
  xhci_dev.doorbell_regs =
      (volatile uint32_t *)(xhci_dev.base_mmio + xhci_dev.cap_regs->Dboff);
