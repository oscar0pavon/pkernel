...
static const struct GuidName known_types[] = {
    ...
    {{0x16, 0xE3, 0xC9, 0xE3, 0x5C, 0x0B, 0xB8, 0x4D, 0x81, 0x7D, 0xF9, 0x2D,
      0xF0, 0x02, 0x15, 0xAE},
     "MS reserved"},
    ...
    {{0x6D, 0xFD, 0x57, 0x06, 0xAB, 0xA4, 0xC4, 0x43, 0x84, 0xE5, 0x09, 0x33,
      0xC8, 0x4B, 0x4F, 0x4F},
     "Linux swap"},
    // Linux LVM             E6D6D379-F507-44C2-A23C-238F2A3DF928
    {{0x79, 0xD3, 0xD6, 0xE6, 0x07, 0xF5, 0xC2, 0x44, 0xA2, 0x3C, 0x23, 0x8F,
      0x2A, 0x3D, 0xF9, 0x28},
     "Linux LVM"},
};
...
const char *gpt_type_name(const uint8_t type_guid[16]) {
  for (unsigned k = 0; k < sizeof(known_types) / sizeof(known_types[0]); k++) {
    int match = 1;
    for (int b = 0; b < 16; b++)
      if (type_guid[b] != known_types[k].guid[b]) {
        match = 0;
        break;
      }
    if (match)
      return known_types[k].name;
  }
  return 0;
}
...
void gpt_guid_str(const uint8_t g[16], char *out) {
  out = put_byte(out, g[3]);
  out = put_byte(out, g[2]);
  out = put_byte(out, g[1]);
  out = put_byte(out, g[0]);
  *out++ = '-';
  out = put_byte(out, g[5]);
  out = put_byte(out, g[4]);
  *out++ = '-';
  out = put_byte(out, g[7]);
  out = put_byte(out, g[6]);
  *out++ = '-';
  out = put_byte(out, g[8]);
  out = put_byte(out, g[9]);
  *out++ = '-';
  for (int i = 10; i < 16; i++)
    out = put_byte(out, g[i]);
  *out = '\0';
}
...
int gpt_scan(BlockDevice *dev, GptPartition *out, int max) {
  ...
  if (ent_size < 128 || ent_size > 512)
    return -1; // sane bounds
  if (num_ent > GPT_MAX_PARTITIONS)
    num_ent = GPT_MAX_PARTITIONS;
  ...
  while (processed < num_ent && found < max) {
    if (block_read(dev, lba, spc, buf) != 0)
      break;

    for (uint32_t i = 0;
         i < ent_per_chunk && processed < num_ent && found < max;
         i++, processed++) {
      ...
      for (c = 0; c < 36; c++) {
        uint16_t ch =
            (uint16_t)e[56 + c * 2] | ((uint16_t)e[56 + c * 2 + 1] << 8);
        if (ch == 0)
          break;
