...
// ACPI Power Management Timer PMTimer
// 
...
#define LAPIC_TIMER_DCR lapic_reg(0x3E0) // Divide Config
  ...
  *LAPIC_LVT_TIMER = (1U << 16) | 0x20;
  ...
  if (pm_port) {
    ...
    *LAPIC_TIMER_ICR = 0xFFFFFFFF;
    ...
    do {
      elapsed_pm =
          (input(pm_port) - pm_start) & pm_mask; // masked sub handles wrap
...
uint64_t lapic_timer_get_ticks(void) { return ticks; }
