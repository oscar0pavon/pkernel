...
BlockDevice *block_register(
  ...
  int i = 0;
  for (; name[i] && i < (int)sizeof(d->name) - 1; i++) d->name[i] = name[i];
  d->name[i]      = '\0';
  ...
  printf("block: registered %s (%lu sectors x %u bytes, %s)\n",
         d->name, sector_count, sector_size, write ? "rw" : "ro");
  return d;
}

BlockDevice *block_get(const char *name) {
  for (int i = 0; i < block_device_count; i++)
    if (name_eq(block_devices[i].name, name)) return &block_devices[i];
  return NULL;
}
...
int block_write(BlockDevice *dev, uint64_t lba, uint32_t count, const void *buf) {
  if (!dev || !dev->write) return -1;  // read-only or invalid device
  if (count == 0) return 0;
  if (lba + count > dev->sector_count) {
