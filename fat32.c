...
static uint32_t fat_next(Fat32Volume *vol, uint32_t cluster) {
  ...
  uint8_t buf[512];
  if (block_read(vol->dev, vol->part_lba + sec, 1, buf) != 0)
    return FAT_EOC;
...
static void fill_entry(const uint8_t *de, const char *name,
                       Fat32DirEntry *out) {
  int i = 0;
  for (; name[i]; i++)
    out->name[i] = name[i];
...
static int entry_skip(const uint8_t *de) {
  if (de[0] == 0xE5)
    return 1; // deleted
  if (de[11] == 0x0F)
    return 1; // long-name (VFAT) component
...
int fat32_mount(BlockDevice *dev, uint64_t part_lba, Fat32Volume *vol) {
  uint8_t bs[512];
  if (block_read(dev, part_lba, 1, bs) != 0)
    return -1;
  if (bs[510] != 0x55 || bs[511] != 0xAA)
    return -1; // boot signature

  uint32_t bps = rd16(bs + 11);
  if (bps != 512)
    return -1; // only 512 B sectors
  uint32_t spc = bs[13];
  if (spc == 0)
    return -1;
  uint32_t fat_sectors = rd32(bs + 36); // BPB_FATSz32
  if (fat_sectors == 0)
    return -1; // 0 => FAT12/16, not us
...
static int dir_find(Fat32Volume *vol, uint32_t cluster, const char *name,
  ...
  while (cluster >= 2 && cluster < FAT_EOC) {
    ...
    for (uint32_t s = 0; s < vol->sectors_per_cluster; s++) {
      if (block_read(vol->dev, vol->part_lba + base + s, 1, buf) != 0)
        return -1;
...
int fat32_stat(Fat32Volume *vol, const char *path, Fat32DirEntry *out) {
  ...
  while (*path) {
    char comp[13];
    int n = 0;
    while (*path && *path != '/' && n < 12)
      comp[n++] = *path++;
    comp[n] = '\0';
    while (*path == '/')
      path++;

    if (dir_find(vol, cluster, comp, &e) != 0)
      return -1;
...
int fat32_read(Fat32Volume *vol, uint32_t first_cluster, uint32_t size,
  ...
  while (done < want && cluster >= 2 && cluster < FAT_EOC) {
    ...
    for (uint32_t s = 0; s < vol->sectors_per_cluster && done < want; s++) {
      if (block_read(vol->dev, vol->part_lba + base + s, 1, sec) != 0)
        return -1;
